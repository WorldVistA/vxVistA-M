VFDDICRW ;SFISC/XAK-SELECT A FILE ;02/12/2014  09:45
 ...
 ;DSS/RAC Cloned from DICRW - to capture files that a user can enter/edit
 Q
 ;FROM DIB
VFD ;
 W !! K DLAYGO,DTOUT D W I '$T!($D(DTOUT))!(Y<0) K DICS,DIA Q:Y<0
 G 1^DIB
 ;
...
DTNOLF ; DT entry point without doing a line feed.
 ...
 K %DT S:$D(IO(0))[0 IO(0)=$I D NOW^%DTC S DT=X,U="^"
 K DIK,DIC,%I,DICS Q
 ;
0 S X=0
1 D:'$D(DISYS) OS^DII
 Q
W D DT
 ; Get users list of files to enter/edit
 N VFD,VFDLST,VX
 D GET^VFDCXPR(.VFDLST,DUZ_";VA(200,~VFD FM FILE SELECT~1")
 I VFDLST(1)["-1" W !!,"You are not authorized to Enter/Edit files" S Y=-1 Q
 N DLAYGO,DUOUT,DTOUT,DSS1 S D=$S('$D(DDS1):"INPUT TO",1:DDS1),DIC(0)=$E("L",$D(DLAYGO)>0)_"EQI"
 D W1,DIC
 W:'$T!($D(DTOUT)) !!,"You are not authorized to Enter/Edit the selected file." Q
 G W:'$P(Y,U,3) K DIC Q
W1 ;
 S VX=0 F  S VX=$O(VFDLST(VX)) Q:'VX  D
 . S:VX=1 VFD("S")="I Y="_$P(VFDLST(VX),U)
 . S:VX>1 VFD("S")=VFD("S")_"!(Y="_$P(VFDLST(VX),U)_")"
 S DIC("S")=VFD("S")_" S DIFILE=+Y,DIAC=""WR"" D ^DIAC I %"
 Q
