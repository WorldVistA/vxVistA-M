DVBHCE ; GENERATED FROM 'DVBHINQ UPDATE' INPUT TEMPLATE(#960), FILE 2;12/30/15
 D DE G BEGIN
...
X19 S DVBJC=$S('$D(^DPT(DFN,.32)):"",1:$P(^(0),U,5))
 Q
20 S DW=".32;6",DV="DX",DU="",DLB="L-EOD",DIFLD=.326
...
X23 I X'=DVBJC S DVBJ2=1
 Q
24 D:$D(DG)>9 F^DIE17,DE S DQ=24,DW=".32;4",DV="P25'X",DU="",DLB="L-Char. Ser.",DIFLD=.324
 S DE(DW)="C24^DVBHCE"
 S DU="DIC(25,"
 G RE
...
X31 S DVBSCR=1 D ^DVBHS4 W !,?34,DVBON,"[NTLAST]",DVBOFF X DVBLIT1
 Q
32 D:$D(DG)>9 F^DIE17,DE S DQ=32,DW=".32;19",DV="SX",DU="",DLB="Service NTL Episode",DIFLD=.3285
 S DE(DW)="C32^DVBHCE"
 S DU="Y:YES;N:NO;"
 G RE
...
X33 I $P(^DPT(D0,.32),U,19)'="Y" S Y="@31"
 Q
34 D:$D(DG)>9 F^DIE17,DE S DQ=34,DW=".32;11",DV="DX",DU="",DLB="NTL-EOD",DIFLD=.3292
...
X34 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) S DFN=DA D SER1^DGLOCK I $D(X) K:'$$VALMSE^DGRPMS(DFN,X,0,"MSNTL") X I $D(X) S DGFRDT=X I $D(^DG(43,1)) S SD1=2 D POS^DGINP
 Q
 ;
35 D:$D(DG)>9 F^DIE17,DE S DQ=35,DW=".32;12",DV="DX",DU="",DLB="NTL-RAD",DIFLD=.3293
...
X36 S DFN=DA K:X=$O(^DIC(23,"B","B.E.C.","")) X I $D(X) D SER1^DGLOCK S DGCOMBR=$G(Y) Q
 Q
 ;
37 D:$D(DG)>9 F^DIE17,DE S DQ=37,DW=".32;9",DV="P25'X",DU="",DLB="NTL-Char. Ser.",DIFLD=.329
 S DE(DW)="C37^DVBHCE"
 S DU="DIC(25,"
 G RE
