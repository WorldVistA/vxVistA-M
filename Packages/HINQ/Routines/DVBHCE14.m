...
X17 I $L(DVBCN)=9,(DVBCN?9N),(DVBCN'=$P(^DPT(D0,0),U,9)) W !!,*7,"HINQ claim # is a SSN, does not match patient file SSN  NO UPDATING claim #" R !,?25,"<RET to continue>",DVBQ:DTIME K DVBQ S Y="@22",JP=JP+1
 Q
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW=".31;3",DV="FXO",DU="",DLB="CLAIM NUMBER",DIFLD=.313
 S DQ(18,2)="S Y(0)=Y S Y=$E(Y,1,10)"
 S DE(DW)="C18^DVBHCE14"
 S X=DVBCN
 S Y=X
...
37 D:$D(DG)>9 F^DIE17,DE S DQ=37,DW=".35;1",DV="DXa",DU="",DLB="DATE OF DEATH",DIFLD=.351
 S DE(DW)="C37^DVBHCE14",DE(DW,"INDEX")=1
 S X=Z1
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
C37F1 N X,X1,X2 S DIXR=180 D C37X1(U) K X2 M X2=X D C37X1("O") K X1 M X1=X
 D
...
C37X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C37F2 S DIXR=703 D C37X2(U) K X2 M X2=X D C37X2("O") K X1 M X1=X
 D
 . D FC^DGFCPROT(.DA,2,.351,"KILL",$H,$G(DUZ),.X,.X1,.X2,$G(XQY0)) Q
 K X M X=X2 D
 . D FC^DGFCPROT(.DA,2,.351,"SET",$H,$G(DUZ),.X,.X1,.X2,$G(XQY0)) Q
...
X54 D POW^DVBHUTIL
 Q
55 D:$D(DG)>9 F^DIE17 G ^DVBHCE16
