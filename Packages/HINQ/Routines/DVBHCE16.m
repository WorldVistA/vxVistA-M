...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".52;5",DV="SX",DU="",DLB="POW STATUS INDICATED?",DIFLD=.525
 S DE(DW)="C1^DVBHCE16",DE(DW,"INDEX")=1
...
X25 I $P(DVBP(6),U,4)[" " S Y="@45",JP=JP+1
 Q
26 S DW=".321;1",DV="SX",DU="",DLB="VIETNAM SERVICE INDICATED?",DIFLD=.32101
 S DE(DW)="C26^DVBHCE16"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 S X=$P(DVBP(6),U,4)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X37 S $P(^DPT(DFN,.3),U,2)="",$P(^DPT(DFN,.3),U,14)=""
 Q
38 D:$D(DG)>9 F^DIE17,DE S DQ=38,DW=".3;2",DV="NJ3,0X",DU="",DLB="SERVICE CONNECTED PERCENTAGE",DIFLD=.302
 S DE(DW)="C38^DVBHCE16"
 S X=+$G(DVBDXPCT)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
...
44 S D=0 K DE(1) ;.3721
 S DIFLD=.3721,DGO="^DVBHCE17",DC="6^2.04P^.372^",DV="2.04MP31'X",DW="0;1",DOW="RATED DISABILITIES (VA)",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DIC(31,"
...
X52 I 'DVBSSA S DVBYN="N",DVBXYN=""
 Q
53 D:$D(DG)>9 F^DIE17 G ^DVBHCE18
