...
ICDDX(CODE,CDT,DFN,SRC) ; Return ICD Dx Code Info
 ...
 ;     -1^Error Description
 ;
 ;DSS/SMP - BEGIN MOD - Look up coding system
 ;N X S X=$$ICDDX^ICDEX($S($G(CODE)?1N.N:$$CODEC^ICDEX(80,$G(CODE)),1:$G(CODE)),$G(CDT),1,,$G(SRC))
 N X S X=$$ICDDX^ICDEX($S($G(CODE)?1N.N:$$CODEC^ICDEX(80,$G(CODE)),1:$G(CODE)),$G(CDT),$$VFD(CODE),,$G(SRC))
 ;DSS/SMP - END MOD
 S $P(X,"^",3)=$TR($P(X,"^",3),";","")
 Q X
ICDOP(CODE,CDT,DFN,SRC) ; Return ICD Operation/Procedure Code Info
 ...
 I $G(^%ZOSF("ZVX"))["VX",CODE?1N.N,+CODE'=CODE S CODE=+$$CODEN(CODE)
 N X S X=$$ICDOP^ICDEX($S($G(CODE)?1N.N:$$CODEC^ICDEX(80.1,$G(CODE)),1:$G(CODE)),$G(CDT),$$VFD2(CODE),,$G(SRC))
 ;DSS/SMP - END MOD
 S $P(X,"^",3)=$TR($P(X,"^",3),";","")
 Q X
...
VLTP(IEN,VDT) ; Versioned Description - Long Text (Proc)
 ...
 ;
 ;DSS/SMP - BEGIN MOD
VFD(CODE) ; Get Coding SYSTEM - File80
 I $G(^%ZOSF("ZVX"))'["VX"!($G(CODE)="") Q 1
 S CODE=$S($G(CODE)'?1N.N:+$$CODEN(CODE,80),1:CODE)
 I CODE,$D(^ICD9(CODE)) Q $$GET1^DIQ(80,CODE,1.1,"I")
 Q 1
 ;
VFD2(CODE) ; Get Coding SYSTEM - File 80.1
 I $G(^%ZOSF("ZVX"))'["VX"!($G(CODE)="") Q 2
 I CODE?1N.N,+CODE'=CODE S CODE=+$$CODEN(CODE)
 I CODE,$D(^ICD0(CODE)) Q $$GET1^DIQ(80.1,CODE,1.1,"I")
 Q 2
