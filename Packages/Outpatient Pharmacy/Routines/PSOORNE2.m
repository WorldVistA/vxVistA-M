PSOORNE2 ;BIR/SAB-display finished orders from backdoor ;9/11/06 10:24am
 ;;7.0;OUTPATIENT PHARMACY;**11,21,23,27,32,37,46,84,103,117,131,146,156,210,148,222,238,264,281,289,251,379,391,313**;DEC 1997;Build 5
 ;^PSDRUG( -  221
 ;^YSCL(603.01 - 2697
 ;^PS(50.606 - 2174
 ;^PS(50.7 - 2223
...
PTST S $P(RN," ",25)=" ",PTST=$S($G(^PS(53,+$P(RX0,"^",3),0))]"":$P($G(^PS(53,+$P(RX0,"^",3),0)),"^"),1:""),IEN=IEN+1
 ...
 .S IEN=IEN+1
 .S ^TMP("PSOAO",$J,IEN,0)="(22) Prescription Pad #: "_$P($G(^PSRX(+RXN,21600)),U,2)
 ;DSS/AMC/SMP - END MOD
 D DISP^PSOORNE6
