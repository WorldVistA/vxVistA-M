PSONRXN ;IHS/DSD/JCM - GETS NEXT VALID RX NUMBER ;08/09/93 9:17
 ;;7.0;OUTPATIENT PHARMACY;**5,25,166,268**;DEC 1997;Build 8
 ;
...
AUTO ; Entry point for getting next rx # if autonumbering
 ...
 ; DSS/JCH - BEGIN MODS - Use HEX numbering defined in VFD PSO RX NUMBERING parameter
 I $G(^%ZOSF("ZVX"))["VX" D
 ...
 I '($G(^%ZOSF("ZVX"))["VX") S:PSOI<PSONRXN("LO") PSOI=PSONRXN("LO")
LOOP2 ; Begin rx # loop
 I ($G(^%ZOSF("ZVX"))["VX") F  S PSOI=$$NEXTRX^VFDPSOHX(PSOI) D  Q:'$D(^PSRX("B",PSOI))!PSONEW("QFLG")
 .N PSOBASE S PSOBASE=$$GET^XPAR("SYS","VFD PSO RX NUMBERING"),PSOBASE=$S(PSOBASE]"":PSOBASE,1:"DEC")
 .I $$RXTODEC^VFDPSOHX(PSOI)>$$RXTODEC^VFDPSOHX(PSONRXN("HI"),PSOBASE) D FATAL Q
 .I '$D(^PSRX("B",PSOI)) D FIL21652^VFDPSOHX(PSOI,PSOBASE)
 I '($G(^%ZOSF("ZVX"))["VX") F  S PSOI=PSOI+1 D:PSOI>PSONRXN("HI") FATAL Q:'$D(^PSRX("B",PSOI))!PSONEW("QFLG")
 G:PSONEW("QFLG") AUTOX
 ; DSS/JCH - END MODS
