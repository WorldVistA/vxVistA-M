PSOCAN3 ;BIR/RTR/SAB - auto dc rxs due to death ;2/05/97 2:59pm
 ;;7.0;OUTPATIENT PHARMACY;**15,24,27,32,36,94,88,117,131,146,139,132,223,235,148,249,225,324,251,375,379,372**;DEC 1997;Build 1
...
ADD S DA=RXDA,RXREF=0,PSODFN=+$P(^PSRX(DA,0),"^",2) S:$G(PSOOPT)=3 REA="L"
 ...
 K PSOTPCNZ
 ;DSS/LM/SMP - BEGIN MOD - ;Append "D" to external Rx# if appropriate
 D:$G(^%ZOSF("ZVX"))["VX" VFD
 ;DSS/LM/SMP - END MOD
 I REA="R" D
...
VFD ;DSS/LM - Append "D" to external Rx# of discontinued PRESCRIPTION
 Q:$$GET^XPAR("SYS","VFD COPY EXTERNAL RX",+$G(DUZ(2)),"I")=0  ;Division check
 I $G(DA)=$G(RXDA),$P($G(^PSRX(DA,"STA")),"^")=12
 I $T N VFDXRX S VFDXRX=$P($G(^PSRX(DA,21600)),"^")
 I $L($G(VFDXRX)),$E(VFDXRX,$L(VFDXRX))'="D" S $P(^PSRX(DA,21600),"^")=VFDXRX_"D"
 Q
