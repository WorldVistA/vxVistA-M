...
PT D DOSE2^PSOORFI4
 ...
 ;DSS/AMC/SMP - BEGIN MOD
 I $G(^%ZOSF("ZVX"))["VX",$$VXRX^VFDPSOU1,$G(PSONEW("PRESCRIPTION PAD #"))]"" S IEN=IEN+1 D
 .S ^TMP("PSOPO",$J,IEN,0)="(16) Prescription Pad #: "_PSONEW("PRESCRIPTION PAD #")
 ;DSS/AMC/SMP - END MOD
...
EDT D KV
  ...
  I $G(^%ZOSF("ZVX"))["VX",$$VXRX^VFDPSOU1 S DIR(0)="LO^1:16"
...
ACP ;
 ...
 ;DSS/DLF/SMP - BEGIN MODS
 I $G(^%ZOSF("ZVX"))["VX" D  Q:PSONEW("DFLG")=1
 .I $T(^VFDPSNDC)]"",$$OPON^VFDPSNDC() D
 ..S DRG=PSODRUG("IEN")
 ..S NDC=$TR(PSODRUG("NDC"),"-","")
 ..; ONLY FOR OMH - REMOVE DEFAULT NDC calculated from NDC from drug file
 ..I $$111^VFDPSNDC() K NDC
 ..; END ONLY OMH
 ..D LOOPNDC^VFDPSNDC(DRG,.NDC)
 ..I NDC="^" S PSONEW("DFLG")=1 Q
 ..S (PSODRUG("NDC"),PSONEW("NDC"))=NDC
 ..S PSONEW("SCANNED CODE")=NDC("SCAN")
 ;DSS/DLF/SMP - END MODS
...
15 D 14^PSOORNW2 Q  ;rem
 ;
 ;DSS/AMC/SMP - BEGIN MOD - Ask for RxPad#
16 ;
 N DIR,DA 
 S DIR(0)="52,21600.01",DIR("B")=$G(PSONEW("PRESCRIPTION PAD #"))
 D ^DIR
 I $D(DTOUT)!$D(DUOUT) S PSONEW("DFLG")=1,VALMBCK="Q" Q
 S PSONEW("PRESCRIPTION PAD #")=Y
 Q
 ;DSS/AMC/SMP - END MOD
 ;
DRGMSG ;
