PSOPKIV1 ;BHAM ISC/MHA - validate PKI cert. ; 5/28/15 9:59am
 ;;7.0;OUTPATIENT PHARMACY;**131,146,223,148,249,391,426**;DEC 1997;Build 18
 ;Ref. to ^ORDEA is supported by DBIA 5709
 ;Ref. to ^ORB supported by DBIA 1362
 ;Ref. to ^XUSSPKI supported by DBIA 3539
CER ;
 ; DSS/JCH - BEGIN MODS - Bypass PKI if parameter set
 I $G(^%ZOSF("ZVX"))["VX",$$GET^XPAR("ALL","VFD PSO BYPASS PKI") S PKI=1 Q
 ; DSS/JCH - END MODS
 N PKIRT
 D VERIFY(.PKIRT,ORD)
...
VERIFY(RET,PSIEN)       ;Verify PKI Data
 ...
 ; DSS/JCH - BEGIN MODS - BYPASS PKI VERIFICATION
 I ($G(^%ZOSF("ZVX"))["VX")&$$GET^XPAR("ALL","VFD PSO BYPASS PKI") S RET="OK"
 I '(($G(^%ZOSF("ZVX"))["VX")&$$GET^XPAR("ALL","VFD PSO BYPASS PKI")) D
