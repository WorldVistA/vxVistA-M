PSOORNE5 ;BIR/SAB - display orders from backdoor con't ;5/10/07 8:29am
 ;;7.0;OUTPATIENT PHARMACY;**11,27,32,46,78,99,117,131,146,171,180,210,222,268,206,225,391**;DEC 1997;Build 8
...
RXNCHK S PSOY=$O(PSONEW("OLD LAST RX#","")) I PSOY="" D AUTO^PSONRXN Q
 ...
 .; DSS/JCH - BEGIN MODS - Use base numbering defined in VFD PSO RX NUMBERING
 .I $G(^%ZOSF("ZVX"))["VX" D
 ..N PSOBASE S PSOBASE=$$GET^XPAR("SYS","VFD PSO RX NUMBERING"),PSOBASE=$S(PSOBASE]"":PSOBASE,1:"DEC")
 ..S PSONRXN("HI")=$P(PSOX1,"^",2)
 ..S PSOI=$P(PSOX1,"^",3)
 ..S PSONEW("OLD LAST RX#",PSONRXN("TYPE"))=PSOI
 ..S:$$RXTODEC^VFDPSOHX(PSOI)<$$RXTODEC^VFDPSOHX(PSONRXN("LO"),PSOBASE) PSOI=PSONRXN("LO")
 .I '(($G(^%ZOSF("ZVX"))["VX")) S PSONRXN("HI")=$P(PSOX1,"^",2),PSOI=$P(PSOX1,"^",3),PSONEW("OLD LAST RX#",PSONRXN("TYPE"))=PSOI D
 ..S:PSOI<PSONRXN("LO") PSOI=PSONRXN("LO")
 .; DSS/JCH - END MODS
 ...
 ; DSS/JCH - BEGIN MODS - Get next RX from alternate base numbering
 Q
LOOP2 I '$G(^%ZOSF("ZVX"))["VX" F  S PSOI=PSOI+1 D:PSOI>PSONRXN("HI") FATAL^PSONRXN Q:'$D(^PSRX("B",PSOI))!PSONEW("QFLG")
 I $G(^%ZOSF("ZVX"))["VX" F  S PSOI=$$NEXTRX^VFDPSOHX(PSOI) D  Q:'$D(^PSRX("B",PSOI))!PSONEW("QFLG")
 .N PSOBASE S PSOBASE=$$GET^XPAR("SYS","VFD PSO RX NUMBERING"),PSOBASE=$S(PSOBASE]"":PSOBASE,1:"DEC")
 .I $$RXTODEC^VFDPSOHX(PSOI)>$$RXTODEC^VFDPSOHX(PSONRXN("HI"),PSOBASE) D FATAL^PSONRXN Q
 .I '$D(^PSRX("B",PSOI)) D FIL21652^VFDPSOHX(PSOI,PSOBASE)
 ; DSS/JCH - END MODS
