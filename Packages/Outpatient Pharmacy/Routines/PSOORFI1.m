PSOORFI1 ;BIR/SAB - finish OP orders from OE/RR continued ;5/23/05 2:11pm
 ;;7.0;OUTPATIENT PHARMACY;**7,15,23,27,32,44,51,46,71,90,108,131,152,186,210,222,258,260,225,391,408**;DEC 1997;Build 5
...
DRG I $P($G(^PSDRUG(+$G(PSODRUG("IEN")),"CLOZ1")),"^")="PSOCLO1" D CLOZ^PSOORFI2
 ...
 I $G(^%ZOSF("ZVX"))["VX",$$VXRX^VFDPSOU1 D  Q:$G(PSONEW("DFLG"))
 .N DIR,DA S DIR(0)="52,21600.01" D ^DIR
 .I $D(DUOUT)!$D(DTOUT) D  Q
 ..S PSONEW("PRESCRIPTION PAD #")="",PSONEW("DFLG")=1,VALMBCK="Q"
...
PST D DOSE^PSOORFI4 K PSOINSFL
 ...
 ;DSS/AMC/SMP - BEGIN MOD
 I $G(^%ZOSF("ZVX"))["VX",$$VXRX^VFDPSOU1 D
 .S IEN=IEN+1,^TMP("PSOPO",$J,IEN,0)="(16) Prescription Pad #: "_$G(PSONEW("PRESCRIPTION PAD #"))
 ;DSS/AMC/SMP - END MOD
