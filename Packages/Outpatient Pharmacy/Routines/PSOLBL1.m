PSOLBL1 ;BHAM ISC/SAB - PRINTS LABEL ; 6/21/13 2:55pm
 ;;7.0;OUTPATIENT PHARMACY;**107,110,225,326,367,318**;DEC 1997;Build 29
START S COPIES=COPIES-1
 ;DSS/SMP - BEGIN MODS - Conditionally remove VA(119)
 I $G(VFDPSOLB) D SVC^VFDPSOLB("LBL1") I 1
 E  W $C(13) S $X=0 W "VA (119)",?10,$E(DT,4,5),"/",$E(DT,6,7),"/",$E(DT,2,3) W:('SIDE)&(PRTFL) ?40,"PLEASE REFER ONLY TO '",$S(REF:"1. REFILL REQUEST",1:"2. RENEWAL ORDER"),"'" W:+$G(RXP) ?100,"(PARTIAL)" W:$G(REPRINT) ?110,"(REPRINT)"
 ;DSS/SMP - END MODS
 ...
 W !
 ;DSS/SMP - BEGIN MODS - remove reference to VA Medical Center
