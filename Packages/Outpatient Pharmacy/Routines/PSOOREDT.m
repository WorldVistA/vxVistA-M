PSOOREDT ;BIR/SAB - edit orders from backdoor ;5/8/08 3:27pm
 ;;7.0;OUTPATIENT PHARMACY;**4,20,27,37,57,46,78,102,104,119,143,148,260,281,304,289,298,379,377,391,313**;DEC 1997;Build 5
...
SEL K PSOISLKD,PSOLOKED S PSOPLCK=$$L^PSSLOCK(PSODFN,0) I '$G(PSOPLCK) D LOCK^PSOORCPY D SVAL K PSOPLCK S VALMBCK="" Q
 ...
 N VFD I $G(^%ZOSF("ZVX"))["VX",$$VXRX^VFDPSOU1 D
 .I $$RXPAD^VFDPSOU1($P(PSOLST(ORN),"^",2)) S VFD=1
 ;S DIR(0)="LO^1:"_$S($$STATUS^PSOBPSUT($P(PSOLST(ORN),"^",2))'="":21,$G(REF):20,1:19)
 S DIR(0)="LO^1:"_$S($G(VFD):22,$$STATUS^PSOBPSUT($P(PSOLST(ORN),"^",2))'="":21,$G(REF):20,1:19)
 ;DSS/AMC/SMP - END MOD
...
EDT ; Rx Edit (Backdoor)
 ...
 .S PSORXED("PRESCRIPTION PAD #")=$P($G(^PSRX($P(PSOLST(ORN),"^",2),21600)),U,2)
 ;DSS/AMC/SMP - END MOD
 ...
 .;DSS/AMC/SMP - BEGIN MOD
 .I FLN=22,$G(^%ZOSF("ZVX"))["VX",$$VXRX^VFDPSOU1,$G(PSORXED("PRESCRIPTION PAD #"))]"" D
 ..N DIR,DA
 ..S DIR(0)="52,21600.01",DIR("B")=PSORXED("PRESCRIPTION PAD #")
 ..K:DIR("B")="" DIR("B") D ^DIR
 ..Q:X=""!(X=PSORXED("PRESCRIPTION PAD #"))
 ..S PSORXED("PRESCRIPTION PAD #")=X
 ..N VFDA S VFDA(52,PSORXED("IRXN")_",",21600.01)=X D FILE^DIE(,"VFDA")
 .;DSS/AMC/SMP - END MOD
