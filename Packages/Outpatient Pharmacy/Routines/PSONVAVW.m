PSONVAVW ;BHM/MFR - View Non-VA Med - Listmanager ;10/20/06
 ;;7.0;OUTPATIENT PHARMACY;**260**;13 Feb 97;Build 8
...
HDR      ; - Header
 ...
 .I VFD S WT=$J($$LB2KG^VFDXLF($P(X,"^",8),2),6),WTDT=$$DT($P(X,"^")\1) Q
 .S WT=$J($P(X,"^",8)/2.2,6,2),WTDT=$$DT($P(X,"^")\1)
 S (HT,X)="",GMRVSTR="HT" D EN6^GMRVUTL I X'="" D
 .I VFD S HT=$J($$IN2CM^VFDXLF($P(X,"^",8),2),6),HTDT=$$DT($P(X,"^")\1) Q
 .S HT=$J($P(X,"^",8)*2.54,6,2),HTDT=$$DT($P(X,"^")\1)
 ;DSS/SMP - END MODS
