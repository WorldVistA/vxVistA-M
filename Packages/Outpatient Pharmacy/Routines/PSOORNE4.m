PSOORNE4 ;BIR/SAB-display renew RXs from backdoor ;07/29/96
 ;;7.0;OUTPATIENT PHARMACY;**11,27,32,36,46,75,96,103,99,117,131,225,386,390,391,313**;DEC 1997;Build 5
...
EDT D KV^PSOVER1 S DIR("A")="Select Field to Edit by number"
 I $G(^%ZOSF("ZVX"))["VX" D
 .S DIR(0)="LO^1:"_$S($$VXRX^VFDPSOU1:11,$G(PSOREEDT):10,1:8)
 E  S DIR(0)="LO^1:"_$S($G(PSOREEDT):10,1:8)
 ;DSS/AMC/SMP - END MOD
...
RMK S IEN=IEN+1,^TMP("PSOPO",$J,IEN,0)="  (8)        Remarks: "_$S($G(PSONEW("REMARKS"))]"":PSONEW("REMARKS"),1:"")
 ;DSS/AMC/SMP - BEGIN MOD
 I $G(^%ZOSF("ZVX"))["VX",$$VXRX^VFDPSOU1 S IEN=IEN+1 D
 .S ^TMP("PSOPO",$J,IEN,0)="  (11) Prescription Pad #: "_$S($G(PSONEW("PRESCRIPTION PAD #"))]"":PSONEW("PRESCRIPTION PAD #"),1:"")
 ;DSS/AMC/SMP - END MOD
...
10 D INS^PSODIR(.PSONEW),SINS^PSODIR(.PSONEW) D SV Q
 ;DSS/AMC/SMP - BEGIN MOD
11 ; Ask for RxPad#
 N DIR,DA S DIR(0)="52,21600.01",DIR("B")=$G(PSONEW("PRESCRIPTION PAD #"))
 K:DIR("B")="" DIR("B") D ^DIR
 I $D(DUOUT)!$D(DTOUT) S PSODIR("DFLG")=1,VALMBCK="Q" Q
