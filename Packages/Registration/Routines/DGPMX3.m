DGPMX3 ; GENERATED FROM 'DGPM DISCHARGE' INPUT TEMPLATE(#448), FILE 405;12/30/15
 D DE G BEGIN
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 ...
 I $D(^("ODS")) S %Z=^("ODS") S %=$P(%Z,U,2) S:%]"" DE(15)=% S %=$P(%Z,U,3) S:%]"" DE(17)=% S %=$P(%Z,U,5) S:%]"" DE(19)=% S %=$P(%Z,U,6) S:%]"" DE(21)=%
 I $D(^("USR")) S %Z=^("USR") S %=$P(%Z,U,3) S:%]"" DE(24)=% S %=$P(%Z,U,4) S:%]"" DE(28)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X18 S Y="@31"
 Q
19 S DW="ODS;5",DV="S",DU="",DLB="DISPLACED FOR ODS PATIENT?",DIFLD=11500.05
 S DU="1:YES;0:NO;"
 G RE
X19 Q
...
X20 S:'X Y="@31"
 Q
21 S DW="ODS;6",DV="NJ14,2",DU="",DLB="VA COST TO TRAVEL",DIFLD=11500.06
 S DE(DW)="C21^DGPMX3"
 G RE
...
C21S S X="" G:DG(DQ)=X C21F1 K DB
 ...
 S A1B2TAG="ADM" D ^A1B2XFR
C21F1 Q
X21 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>99999999999)!(X<0) X
 Q
 ;
22 S DQ=23 ;@31
23 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=23 D X23 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X23 I DGPMP=^DGPM(DA,0) S Y=""
 Q
24 D:$D(DG)>9 F^DIE17,DE S DQ=24,DW="USR;3",DV="RP200'",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DU="VA(200,"
 S X=DUZ
 S Y=X
...
27 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=27 G A
28 S DW="USR;4",DV="RD",DU="",DLB="LAST EDITED ON",DIFLD=103
 S %=$P($H,",",2),X=DT_(%\60#60/100+(%\3600)+(%#60/10000)/100)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X28 S %DT="STX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
29 G 0^DIE17
