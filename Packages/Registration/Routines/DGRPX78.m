...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.3)) S %Z=^(.3) S %=$P(%Z,U,9) S:%]"" DE(27)=% S %=$P(%Z,U,11) S:%]"" DE(17)=%
...
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".385;2",DV="*P27.18'X",DU="",DLB="PENSION AWARD REASON",DIFLD=.3852
 S DE(DW)="C4^DGRPX78"
 S DU="DG(27.18,"
 S X="@"
 S Y=X
...
X5 S Y="@7026"
 Q
6 S DQ=7 ;@7024
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".385;2",DV="*P27.18'X",DU="",DLB="PENSION AWARD REASON",DIFLD=.3852
 S DE(DW)="C7^DGRPX78"
 S DU="DG(27.18,"
 G RE
...
X15 D EN^DDIOL("other assistance, use the HEC Alert process.","","!")
 Q
16 S DQ=17 ;@7026
17 D:$D(DG)>9 F^DIE17,DE S DQ=17,DW=".3;11",DV="SX",DU="",DLB="RECEIVING VA DISABILITY?",DIFLD=.3025
 S DE(DW)="C17^DGRPX78"
...
X18 S:(X'="Y")&($P($G(^DPT(DA,.362)),U,12,14)'["Y") Y=.36265
 Q
19 D:$D(DG)>9 F^DIE17,DE S DQ=19,DW=".362;20",DV="NJ8,2X",DU="",DLB="TOTAL ANNUAL VA CHECK AMOUNT",DIFLD=.36295
 S DE(DW)="C19^DGRPX78"
 G RE
...
X19 D DOL^DGLOCK2 K:+X'=X&(X'?.N1"."2N)!(X>99999)!(X<0) X I $D(X) S DFN=DA D MV^DGLOCK I $D(X),('$$TOTCHK^DGLOCK2(DFN)) D TOTCKMSG^DGLOCK2 K X
 Q
 ;
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW=".362;17",DV="SX",DU="",DLB="GI INSURANCE POLICY?",DIFLD=.36265
 S DE(DW)="C20^DGRPX78"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X21 S:X'="Y" Y="@703"
 Q
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW=".362;6",DV="NJ8,2X",DU="",DLB="AMOUNT OF GI INSURANCE",DIFLD=.3626
 G RE
...
X24 S:DGDR'["703" Y="@704"
 Q
25 S DW=".36;1",DV="*P8'X",DU="",DLB="PRIMARY ELIGIBILITY CODE",DIFLD=.361
 S DE(DW)="C25^DGRPX78",DE(DW,"INDEX")=1
 S DU="DIC(8,"
 G RE
...
C25S S X="" G:DG(DQ)=X C25F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 D AUTOUPD^DGENA2(DA)
C25F1 N X,X1,X2 S DIXR=840 D C25X1(U) K X2 M X2=X D C25X1("O") K X1 M X1=X
 D
...
X26 D AAC1^DGLOCK2 S:DGAAC(1)']"" Y=361
 Q
27 D:$D(DG)>9 F^DIE17,DE S DQ=27,DW=".3;9",DV="*P35'X",DU="",DLB="AGENCY/ALLIED COUNTRY",DIFLD=.309
 S DU="DIC(35,"
 G RE
X27 S DFN=DA D AAC^DGLOCK2
 ...
 ;
28 S D=0 K DE(1) ;361
 S DIFLD=361,DGO="^DGRPX79",DC="3^2.0361IP^E^",DV="2.0361M*P8'X",DW="0;1",DOW="ELIGIBILITY",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DIC(8,"
...
R28 D DE
 ...
 ;
29 S DW=".32;3",DV="*P21'X",DU="",DLB="PERIOD OF SERVICE",DIFLD=.323
 S DE(DW)="C29^DGRPX78"
 S DU="DIC(21,"
 G RE
...
X30 D ^DGYZODS S:'DGODS Y="@704"
 Q
31 D:$D(DG)>9 F^DIE17,DE S DQ=31,DW="ODS;2",DV="S",DU="",DLB="RECALLED TO ACTIVE DUTY",DIFLD=11500.02
 S DE(DW)="C31^DGRPX78"
...
C31F1 Q
X31 Q
32 D:$D(DG)>9 F^DIE17,DE S DQ=32,DW="ODS;3",DV="*P25002.1'",DU="",DLB="RANK",DIFLD=11500.03
 S DE(DW)="C32^DGRPX78"
 S DU="DIC(25002.1,"
 G RE
...
35 D:$D(DG)>9 F^DIE17,DE S DQ=35,D=0 K DE(1) ;.3731
 S DIFLD=.3731,DGO="^DGRPX710",DC="2^2.05A^.373^",DV="2.05MFX",DW="0;1",DOW="SERVICE CONNECTED CONDITIONS",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
