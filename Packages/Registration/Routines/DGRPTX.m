DGRPTX ; GENERATED FROM 'DGRPT 10-10T REGISTRATION' INPUT TEMPLATE(#1476), FILE 2;12/30/15
 D DE G BEGIN
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,2) S:%]"" DE(3)=%
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,1) S:%]"" DE(7)=% S %=$P(%Z,U,2) S:%]"" DE(9)=% S %=$P(%Z,U,3) S:%]"" DE(11)=% S %=$P(%Z,U,4) S:%]"" DE(15)=% S %=$P(%Z,U,5) S:%]"" DE(17)=% S %=$P(%Z,U,12) S:%]"" DE(14)=%
...
X2 I $G(DGNEWPF) S Y="@10"
 Q
3 S DW="0;2",DV="RS",DU="",DLB="SEX",DIFLD=.02
...
X5 I $D(X) S:'$D(DPTX) DFN=DA D:'$D(^XUSEC("DG ELIGIBILITY",DUZ)) VAGE^DGLOCK:X="Y" I $D(X) D:$D(DFN) EV^DGLOCK
 ...
 ;
6 S DQ=7 ;@10
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".11;1",DV="F",DU="",DLB="STREET ADDRESS [LINE 1]",DIFLD=.111
...
X8 S:X="" Y="@1112"
 Q
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW=".11;2",DV="F",DU="",DLB="STREET ADDRESS [LINE 2]",DIFLD=.112
...
X10 S:X="" Y="@1112"
 Q
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW=".11;3",DV="F",DU="",DLB="STREET ADDRESS [LINE 3]",DIFLD=.113
...
X13 S EASZIPLK=1
 Q
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,DW=".11;12",DV="FXO",DU="",DLB="ZIP+4",DIFLD=.1112
...
X14 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>20!($L(X)<5) X I $D(X) D ZIPIN^VAFADDR
 ...
 Q
 ;
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW=".11;4",DV="F",DU="",DLB="CITY",DIFLD=.114
...
X16 S:'$$KEY^DGREGDD1(DUZ,DA) Y=.131
 Q
17 D:$D(DG)>9 F^DIE17,DE S DQ=17,DW=".11;5",DV="*P5'",DU="",DLB="STATE",DIFLD=.115
