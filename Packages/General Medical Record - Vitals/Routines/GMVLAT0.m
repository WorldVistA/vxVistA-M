GMVLAT0 ;HOIFO/YH,FT-DISPLAY LATEST VITALS/MEASUREMENTS FOR A PATIENT ;9/27/07
 ;;5.0;GEN. MED. REC. - VITALS;**1,3,23**;Oct 31, 2002;Build 8
 ;
...
EN1(DFN) ; Entry to extract the latest vitals/measurements for a patient
 ...
 I VFD F X="T","P","R","PO2","BP","HT","WT","CVP","CG","PN","LMP","EDD","LEN" I $D(^GMRD(120.51,"C",X))  S GMR(X)=$O(^GMRD(120.51,"C",X,"")),Y=$P($G(^GMRD(120.51,GMR(X),0)),"^") Q:Y=""
 I 'VFD F X="T","P","R","PO2","BP","HT","WT","CVP","CG","PN" I $D(^GMRD(120.51,"C",X))  S GMR(X)=$O(^GMRD(120.51,"C",X,"")),Y=$P($G(^GMRD(120.51,GMR(X),0)),"^") Q:Y=""
 ...
 ;DSS/LM - Modify loop to include vxVistA additional vital types - 2/16/2010 Add LEN
 ;       - OSHERA note. Values added to the FOR list will not be found in VA-VistA
 ;                      Hence the $DATA(GMRDATA(X)) test fails and there is no effect
 ...
 F X="T","P","R","PO2","BP","HT","WT","CVP","CG","PN","LMP","EDD","LEN" I $D(GMRDATA(X)) S GMRVDT="",(GMRVDT(1),GMVD)=0 F  S GMVD=$O(GMRDATA(X,GMVD)) Q:GMVD'>0  D WRTDT S GMVD(1)=0 F  S GMVD(1)=$O(GMRDATA(X,GMVD,GMVD(1))) Q:$L(GMVD(1))'>0  D
 .;DSS/LM - End modification
 ...
 .;DSS/SMP - BEGIN MODS - MEASUREMENT CONVERSIONS
 .I X="WT" D
 ..I VFD D  Q
 ...S ^TMP($J,"GRPC",GJ)=^TMP($J,"GRPC",GJ)_GMRVX(0)_$S(GMVNUMBR=1:" lb  ("_$$LB2KG^VFDXLF(GMRVX(0),2)_" kg)",1:"") S GMRVWT=+$$LB2KG^VFDXLF(GMRVX(0))
 ..S ^TMP($J,"GRPC",GJ)=^TMP($J,"GRPC",GJ)_GMRVX(0)_$S(GMVNUMBR=1:" lb  ("_$J(GMRVX(0)*.45359237,0,2)_" kg)",1:"") S GMRVWT=GMRVX(0)/2.2
 .I X="HT" D
 ..I VFD S GMRVHT=+$$IN2M^VFDXLF(GMRVX(0))
 ..I 'VFD S GMRVHT=(GMRVX(0)*2.54)/100
 ..I GMVNUMBR=0 S ^TMP($J,"GRPC",GJ)=^TMP($J,"GRPC",GJ)_GMRVX(0) Q
 ..I VFD D  Q
 ...S ^TMP($J,"GRPC",GJ)=^TMP($J,"GRPC",GJ)_$S(GMRVX(0)\12:GMRVX(0)\12_" ft ",1:"")_$S(GMRVX(0)#12:GMRVX(0)#12_" in",1:"")_$S(GMVNUMBR=1:" ("_$$IN2CM^VFDXLF(GMRVX(0),2)_" cm)",1:"")
 ...
 .I X="CG" D
 ..I VFD D  Q
 ...S ^TMP($J,"GRPC",GJ)=^TMP($J,"GRPC",GJ)_GMRVX(0)_$S(GMVNUMBR=1:" in ("_$$IN2CM^VFDXLF(+GMRVX(0),2)_" cm)",1:"")
 ..S ^TMP($J,"GRPC",GJ)=^TMP($J,"GRPC",GJ)_GMRVX(0)_$S(GMVNUMBR=1:" in ("_$J(+GMRVX(0)/.3937,0,2)_" cm)",1:"")
 .;DSS/SMP - END MODS
 ...
 .;       - See OSEHRA note at top of FOR loop
