...
CALBMI(GBMI,GMVDEC) ;OBTAIN HEIGHT TO CALCULATE BMI
 ...
 ;
 ;DSS/SMP - BEGIN MODS
VFD(VFDWT,VFDHT) ; Convert height and weight using vxVistA API
 ; Inputs:   VFDWT = Weight in LB
 ;           VFDHT = Height in IN
 ; 
 ; Outputs:  GBMI(2) = Weight in KG
 ;           GMRVHT  = Height in M
 ;
 ; If vxVistA, use the standard API
 ; Else, use the VA's way
 ; 
 I $G(^%ZOSF("ZVX"))["VX" D  I 1
 .S GBMI(2)=+$$LB2KG^VFDXLF(VFDWT)
 .S GMRVHT=+$$IN2M^VFDXLF(VFDHT)
 E  S GBMI(2)=VFDWT/2.2,GMRVHT=VFDHT*2.54/100
 Q
