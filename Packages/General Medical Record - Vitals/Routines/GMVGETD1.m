GMVGETD1 ;HOIFO/YH-EXTRACT VITALS/MEASUREMENT RECORDS FOR A GIVEN DATE ;5/10/07
 ;;5.0;GEN. MED. REC. - VITALS;**23**;Oct 31, 2002;Build 15
 ;
...
EN1 ;CALLED FROM GMVGETD ENTRY POINT GETVM 
 ...
 I $G(^%ZOSF("ZVX"))["VX" F GMRVTY="T","P","R","BP","HT","WT","CVP","CG","PO2","PN","LMP","EDD","LEN"  S GMRVITY=$O(^GMRD(120.51,"C",GMRVTY,0)) I GMRVITY>0 D SETAR
 I $G(^%ZOSF("ZVX"))'["VX" F GMRVTY="T","P","R","BP","HT","WT","CVP","CG","PO2","PN"  S GMRVITY=$O(^GMRD(120.51,"C",GMRVTY,0)) I GMRVITY>0 D SETAR
...
PRT ;SAVE V/M BY DATE/TIME
 ;DSS/LM - Begin modification - Accommodate LMP and EDD - 2/16/2010 Add LEN
 ;       - 8/5/2013 - Add OSEHRA check
 I $G(^%ZOSF("ZVX"))["VX" F GMRVTY="T","P","R","BP","HT","WT","CVP","CG","PO2","PN","LMP","EDD","LEN" I $D(^TMP($J,"GMRV",GMRDATE,GMRVTY)) S GMRVDA=0 F  S GMRVDA=$O(^TMP($J,"GMRV",GMRDATE,GMRVTY,GMRVDA)) Q:$L(GMRVDA)'>0  D SETLN^GMVGETD2
 I $G(^%ZOSF("ZVX"))'["VX" F GMRVTY="T","P","R","BP","HT","WT","CVP","CG","PO2","PN" I $D(^TMP($J,"GMRV",GMRDATE,GMRVTY)) S GMRVDA=0 F  S GMRVDA=$O(^TMP($J,"GMRV",GMRDATE,GMRVTY,GMRVDA)) Q:$L(GMRVDA)'>0  D SETLN^GMVGETD2
