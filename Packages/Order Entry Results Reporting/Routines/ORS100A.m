...
LOOP ;called from ORS100
 ...
 ;DSS/RAC - BEGING MOD - SET IF VX ACCOUNT
 N VFD
 S VFD=0
 S:^%ZOSF("ZVX")["VX" VFD=1
 ;DSS/RAC - END MOD
 ...
 .....;DSS/RAC - BEGIN MOD - use MRN verses SSN if vxVistA Original line commented
 .....;S DFN=+$P(^OR(100,IEN,0),U,2) D DEM^VADPT S SSN=VA("BID"),PNM=$E(VADM(1),1,24)
 .....S DFN=+$P(^OR(100,IEN,0),U,2) D DEM^VADPT
 .....I VFD S SSN=$E(VA("MRN"),$L(VA("MRN"))-23,$L(VA("MRN"))),PNM=$E(VADM(1),1,24)
 .....E  S SSN=VA("BID"),PNM=$E(VADM(1),1,24)
 .....;DSS/RAC -END MOD
...
PROV ;loops thru the TMP global for output sort by provider
 ...
 ....I VFD,'SUMONLY W $P(^(IEN),U),?25,$P(^(IEN),U,2),?50,$P(^(IEN),U,3),?75,$P(^(IEN),U,4),?103,$P(^(IEN),U,5),?108,$P(^(IEN),U,6),?120,$P(^(IEN),U,7),! D:$Y>(IOSL-4) HDR^ORS100 Q:STOP
...
PAT ;loops thru the TMP global for output sort by patient
 ...
 .;DSS/RAC - BEGIN MODS - if vxVistA user MRN verses SSN original VA code in else
 .I VFD S HDR1="!!,""PATIENT"",?25,""MRN(Last 25)"",?50,""PROVIDER"",?75,""ENTERED BY"",?101,""STATUS"",?108,""ORDER #"",?120,""ORDER DATE"""
 .E  S HDR1="!!,""PATIENT"",?25,""SSN"",?30,""PROVIDER"",?55,""ENTERED BY"",?81,""STATUS"",?95,""ORDER #"",?110,""ORDER DATE"""
 .;DSS/RAC - END MOD
 .S PAGE=0 D HDR^ORS100
 .I '$D(^TMP("ORUNS",$J)) W !,"No unsigned orders found" Q
 ...
 ....;DSS/RAC - BEGIN MOD - Change spacing to a accomodate MRN (orgininal in else)
 ....I VFD,'SUMONLY W $P(^(IEN),U),?25,$P(^(IEN),U,2),?50,$P(^(IEN),U,3),?75,$P(^(IEN),U,4),?103,$P(^(IEN),U,5),?108,$P(^(IEN),U,6),?120,$P(^(IEN),U,7),! D:$Y>(IOSL-4) HDR^ORS100 Q:STOP
 ....E  I 'SUMONLY W $P(^(IEN),U),?25,$P(^(IEN),U,2),?30,$P(^(IEN),U,3),?55,$P(^(IEN),U,4),?81,$P(^(IEN),U,5),?95,$P(^(IEN),U,6),?110,$P(^(IEN),U,7),! D:$Y>(IOSL-4) HDR^ORS100 Q:STOP
 ....;DSS/RAC - END MOD
...
SPAT ;sorts for a single patient
 ...
 .I TYPE=3 S HDR="!!?30,""List of UNSIGNED/UNRELEASED orders for "",LONER"
 .;DSS/RAC - BEGIN MODS - if vxVistA user MRN verses SSN original VA code in else
 .I VFD S HDR1="!!,""PATIENT"",?25,""MRN(Last 25)"",?60,""PROVIDER"",?75,""ENTERED BY"",?101,""STATUS"",?108,""ORDER #"",?120,""ORDER DATE"""
 .E  S HDR1="!!,""PATIENT"",?25,""SSN"",?30,""PROVIDER"",?55,""ENTERED BY"",?81,""STATUS"",?95,""ORDER #"",?110,""ORDER DATE"""
 .;DSS/RAC - END MOD
 .D HDR^ORS100
...
WARD ;loops thru the TMP global for output sort by location
 ...
 .;DSS/RAC - BEGIN MODS - if vxVistA user MRN verses SSN original VA code in else
 .I VFD S HDR1="!!,""PROVIDER"",?25,""ENTERED BY"",?50,""PATIENT"",?75,""MRN(Last 23)"",?101,""STATUS"",?108,""ORDER #"",?120,""ORDER DATE"""
 .E  S HDR1="!!,""PROVIDER"",?25,""ENTERED BY"",?50,""PATIENT"",?75,""SSN"",?81,""STATUS"",?95,""ORDER #"",?110,""ORDER DATE"""
 .;DSS/RAC - END MOD
 .S PAGE=0 D HDR^ORS100
 .I '$D(^TMP("ORUNS",$J)) W !,"No unsigned orders found" Q
 ...
 ....;DSS/RAC - BEGIN MOD - Change spacing to a accomodate MRN (orgininal in else)
 ....I VFD,'SUMONLY W $P(^TMP("ORUNS",$J,LOC,PROV,IEN),U,2),?25,$P(^(IEN),U,3),?50,$P(^(IEN),U,4),?75,$P(^(IEN),U,5),?105,$P(^(IEN),U,6),?108,$P(^(IEN),U,7),?120,$P(^(IEN),U,8),! D:$Y>(IOSL-4) HDR^ORS100 Q:STOP
 ....E  I 'SUMONLY W $P(^TMP("ORUNS",$J,LOC,PROV,IEN),U,2),?25,$P(^(IEN),U,3),?50,$P(^(IEN),U,4),?75,$P(^(IEN),U,5),?81,$P(^(IEN),U,6),?95,$P(^(IEN),U,7),?110,$P(^(IEN),U,8),! D:$Y>(IOSL-4) HDR^ORS100 Q:STOP
 ....;DSS/RAC - END MOD
