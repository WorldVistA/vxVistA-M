ORQPTQ2 ; slc/CLA - Functions which return patient lists and list sources pt 2 ;3/14/05  10:50
 ;;3.0;ORDER ENTRY/RESULTS REPORTING;**9,10,85,187,190,195,215,320**;Dec 17, 1997;Build 18
 ;
...
WARD(Y) ; RETURN LIST OF ACTIVE WARDS
 ...
 . ;DSS/SMP - BEGIN MODS - add checks for parameter VFD WARD RESTRICTION
 . D VFD(IEN)
 . ;DSS/SMP - END MODS
...
NLIST(ORQY) ; return a null list
 ...
 Q
 ;
 ;DSS/SMP - BEGIN MODS - Ward Restriction
VFD(WARD) ; Ward Restriction
 ; Leave X=0 to add the ward
 ; Set X=1 to prevent the ward from being added
 ;
 N INST,LIST
 I $G(^%ZOSF("ZVX"))'["VX" Q  ; OSEHRA check
 D GET^VFDCXPR(.LIST,"~VFD WARD RESTRICTION~1")
 Q:'$P(LIST(1),U,3)  K LIST
 S INST=$$GET1^DIQ(42,WARD,".015:.07","I",,"VFDERR")
