ECMXP ; GENERATED FROM 'EC CREATE PATIENT ENTRY' INPUT TEMPLATE(#1503), FILE 721;12/30/15
 D DE G BEGIN
DE S DIE="^ECH(",DIC=DIE,DP=721,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^ECH(DA,""))=""
 ...
 I  S %=$P(%Z,U,19) S:%]"" DE(25)=% S %=$P(%Z,U,20) S:%]"" DE(26)=% S %=$P(%Z,U,22) S:%]"" DE(27)=% S %=$P(%Z,U,23) S:%]"" DE(28)=%
 ...
 I  S %=$P(%Z,U,10) S:%]"" DE(22)=% S %=$P(%Z,U,11) S:%]"" DE(23)=% S %=$P(%Z,U,12) S:%]"" DE(24)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
22 S DW="P;10",DV="S",DU="",DLB="HEAD/NECK CANCER",DIFLD=39
 ...
 G RD:X="@",Z
X22 Q
23 S DW="P;11",DV="S",DU="",DLB="COMBAT VETERAN",DIFLD=40
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 S X=$G(ECPTR("CV"))
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z
X23 Q
24 S DW="P;12",DV="S",DU="",DLB="PROJECT 112/SHAD",DIFLD=41
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 S X=$G(ECPTR("SHAD"))
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z
X24 Q
25 S DW="0;19",DV="*P44'X",DU="",DLB="ASSOCIATED CLINIC",DIFLD=26
 S DU="SC("
 S X=$G(ECPTR("CLIN"))
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z
X25 Q
26 S DW="0;20",DV="P40.7'",DU="",DLB="DSS ID",DIFLD=27
 S DU="DIC(40.7,"
 ...
 G RD:X="@",Z
X26 Q
27 S DW="0;22",DV="RS",DU="",DLB="IN/OUTPATIENT",DIFLD=29
 ...
 G RD:X="@",Z
X27 Q
28 S DW="0;23",DV="*P720.5'",DU="",DLB="PROCEDURE REASON",DIFLD=34
 S DU="ECL("
 S X=$G(ECPRR("REAS"))
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
