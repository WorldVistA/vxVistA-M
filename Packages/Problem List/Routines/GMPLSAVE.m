GMPLSAVE ; ISL/MKB,KER,JER,TC -- Save Problem List data ;06/05/14  08:42
 ;;2.0;Problem List;**26,31,35,37,38,36,42**;Aug 25, 1994;Build 29
 ;
 ; External References
...
EN ; Save Changes made to Existing Problem
 ...
 ;DSS/SMP - BEGIN MODS
 N VFD S VFD=$G(^%ZOSF("ZVX"))["VX" I VFD S (VFD(21600.01),VFD(21600.02))=""
 F FLD=.01,.05,.12,.13,1.01,1.05,1.06,1.07,1.08,1.09,1.1,1.11,1.12,1.13,1.14,1.15,1.16,1.17,1.18,80001,80002,80003,80004,80005,80201,80202 S VFD(FLD)=""
 S FLD=0 F  S FLD=$O(VFD(FLD)) Q:'FLD  D
 . ;DSS/SMP - END MODS
...
NEW ; Save Collected Values in new Problem Entry
 ...
 ;DSS/SMP - BEGIN MODS - Add 21600 Fields
 ;  Set Node 21600
 I $G(^%ZOSF("ZVX"))["VX" D
 . S DATA=$P($G(GMPFLD(21600.01)),U)_U_$P($G(GMPFLD(21600.02)),U)
 . I DATA'=U S ^AUPNPROB(DA,21600)=DATA
 ;DSS/SMP - END MODS
