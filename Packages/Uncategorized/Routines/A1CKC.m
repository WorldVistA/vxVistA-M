A1CKC ; GENERATED FROM 'A1CK VARO/DHCP' INPUT TEMPLATE(#1505), FILE 2;12/30/15
 D DE G BEGIN
...
X10 I $D(X) S:'$D(DPTX) DFN=DA D:'$D(^XUSEC("DG ELIGIBILITY",DUZ)) VAGE^DGLOCK:X="Y" I $D(X) D:$D(DFN) EV^DGLOCK
 Q
 ;
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW=".3;1",DV="SX",DU="",DLB="SERVICE CONNECTED?",DIFLD=.301
 S DE(DW)="C11^A1CKC"
 S DU="Y:YES;N:NO;"
 S Y="N"
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X14 S DFN=DA D MV^DGLOCK I $D(X) S DFN=DA D EV^DGLOCK
 Q
 ;
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW=".36;1",DV="*P8'X",DU="",DLB="PRIMARY ELIGIBILITY CODE",DIFLD=.361
...
C15S S X="" G:DG(DQ)=X C15F1 K DB
 D ^A1CKC4
C15F1 N X,X1,X2 S DIXR=840 D C15X1(U) K X2 M X2=X D C15X1("O") K X1 M X1=X
 D
 . D FC^DGFCPROT(.DA,2,.361,"KILL",$H,$G(DUZ),.X,.X1,.X2,$G(XQY0)) Q
 K X M X=X2 D
 . D FC^DGFCPROT(.DA,2,.361,"SET",$H,$G(DUZ),.X,.X1,.X2,$G(XQY0)) Q
...
X15 S DFN=DA D EV^DGLOCK I $D(X) D ECD^DGLOCK1
 Q
 ;
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,DW="TYPE;1",DV="P391'",DU="",DLB="TYPE",DIFLD=391
