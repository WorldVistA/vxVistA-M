 ...
 ;(C)InterSystems, generated for class MPIPSIM.PSIMWebServicePort.execute.  Do NOT edit. 06/02/2016 02:21:23PM
 ;;4C374B6C;MPIPSIM.PSIMWebServicePort.execute
 ;
...
zXMLExportInternal()
 ...
 Set tag=$get(topArg)
 Set tmpi=(($get(typeAttr)'="")&&(typeAttr'="MPIPSIM.PSIMWebServicePort.execute"))
 If $IsObject(namespaces) {
   If namespaces.Stable,namespaces.CurrentNamespace="PSIM",'tmpi||(typesPrefix'="") {
     Set topAttrs=""
   } Else {
     Set popAtEnd=1,saveTopPrefix=topPrefix,saveTypesPrefix=typesPrefix,saveAttrsPrefix=attrsPrefix,saveUsePrefix=usePrefix
     Set sc=namespaces.PushNodeForExport("PSIM",$get(local,0),1,0,,.topPrefix,.topAttrs,.typesPrefix,.attrsPrefix,.usePrefix)
     If 'sc Quit sc
   }
   Set beginprefix=""
