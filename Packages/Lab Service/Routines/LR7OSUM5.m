LR7OSUM5 ;DALOI/STAFFC - Silent Patient cum cont. ;02/20/13  16:48
 ;;5.2;LAB SERVICE;**121,187,228,241,250,251,256,356,372,350,427**;Sep 27, 1994;Build 18
 ;
TS ; from LR7OSUM3
...
LRHI ;
 ...
 ;DSS/FHS - BEGIN MOD - RESET HI-LO TO FILE 63 most recent VALUES if vertical format
 ; LROFMT=V = VERTICAL PRINT FORMAT
 I $G(LROFMT)'="V" Q
 N VFDSUB,VFDSP,VFDIDT,RNG
 S VFDCNT=$G(VFDCNT)+1,VFDCNT=VFDCNT,VFDSUB=0
 S VFDSP=$P($G(^LAB(64.5,1,1,LRMH,1,LRSH,0)),U,2)
 S ^TMP("OUT",$J,VFDCNT,"A")=+$G(^LAB(64.5,"A",1,LRMH,LRSH,I(I)))_"~"_LRMH_U_LRSH_U_I(I)
 S ^TMP("OUT",$J,VFDCNT,I(I))=VFDSP_U_$G(^LAB(64.5,1,1,LRMH,1,1,LRSH,I(I),0))
 S VFDSUB=+$P($G(^TMP("OUT",$J,VFDCNT,I(I))),";",2)
 I $D(^XTMP("LRNR","U",$J,VFDSUB,VFDSP)) S ^TMP("OUT",$J,VFDCNT,"VAL",VFDSUB)=^(VFDSP) D
 . S VFDIDT=$P($G(^TMP("OUT",$J,VFDCNT,"VAL",VFDSUB)),"~",2)
 . Q:VFDIDT<1
 . S ^TMP("OUT",$J,VFDCNT,"RES",VFDSUB)=$G(^LR(LRDFN,"CH",VFDIDT,VFDSUB))
 . I $D(^LR(LRDFN,"CH",VFDIDT,VFDSUB)) S RNG=$P(^(VFDSUB),U,5) D
 . . S LRLO=$P(RNG,"!",2)
 . . S LRHI=$P(RNG,"!",3)
 . . S LRLOHI=$$EN^LRLRRVF(LRLO,LRHI)
 K ^TMP("OUT",$J)
