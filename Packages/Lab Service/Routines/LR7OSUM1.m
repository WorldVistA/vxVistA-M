LR7OSUM1 ;DALOI/staff - Silent Patient cum cont. ;06/03/13  11:17
 ;;5.2;LAB SERVICE;**121,187,256,286,384,350,427**;Sep 27, 1994;Build 18
 ;
LRIDT ; from LR7OSUM
 N GIOM,LRDRL
 S GIOM=$G(LRGIOM)
...
SBSET ;
 ...
 ;DSS/FHS - BEGIN MOD - Set LROFMT and call LRRPU - If LROFMT="V" report most recent normal range value for ^LR(LRDFN,LRSS,LRIDT,LRSB,0)
 ;
 N LROFMT
 S LROFMT=$P($G(^LAB(64.5,1,1,LRMH,1,LRSH,0)),U,3)
 S LRTRES=$$TSTRES^LRRPU(LRDFN,"CH",LRIDT,LRSUB,"")
 ;DSS/FHS - END MOD - Jan 13, 2015
 ; 
...
REALDIFF() ;
 ...
 ;DSS/FHS - BEGIN MOD - Exchange the normal range values if File 60 values are different for printing CUM
 ;  Exchange noraml range value to file 60 normal range for printing
 S VFDTRES=LRTRES
 I DIFF S $P(LRTRES,U,3)=LRLO,$P(LRTRES,U,4)=LRHI
