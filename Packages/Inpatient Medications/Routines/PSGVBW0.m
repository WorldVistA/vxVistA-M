PSGVBW0 ;BIR/CML3,MV - SHOW NON-VERFIED ORDERS GATHERED IN PSGVBW ;09/17/97 1:41 PM
 ;;5.0;INPATIENT MEDICATIONS;**29,39,53,56,95,80,110,127,124,243**;DEC 16, 1997;Build 29
 ;
...
PROCESS ; Loop through selected patients and display profile/orders.
 ...
 ; DSS/SMH/SMP - BEGIN MODS Only print labels if NDC capture is not turned on
 I $G(^%ZOSF("ZVX"))["VX",$T(^VFDPSNDC)]"" D
 .F PSJCNT=0:0 S PSJCNT=$O(^TMP("PSJSELECT",$J,PSJCNT)) Q:'PSJCNT  D PROCESS1 S PSGOP=PSGP D ENQL^PSGLW:($P(PSJSYSL,"^",2)]""&'$$IPON^VFDPSNDC()) Q:$G(PSJGOTO)="E"  I $D(^TMP("PSJSELECT",$J,+$G(PSJGOTO))) S PSJCNT=PSJGOTO-1
 E  D
