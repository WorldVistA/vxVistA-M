PSGOEE ;BIR/CML3-EDIT ACTIVE OR NON-VERIFIED ORDERS ; 8/12/10 5:59pm
 ;;5.0;INPATIENT MEDICATIONS;**4,7,29,47,64,58,82,91,110,111,112,142,179,181,254,267,268**;16 DEC 97;Build 1
 ;
...
NEW ;
 ...
 ;DSS/LM/SMP - BEGIN MOD - Copy Ext Rx # and Provider Comments
 N VFD S VFD=$G(^%ZOSF("ZVX"))["VX"
 I 'PSGOEAV D:VFD VFD ;Auto-Verify Off
 ;I PSGOEAV,+PSJSYSU=3,'$D(PSGOES) D EN^PSGPEN(PSGORD),UNL^PSSLOCK(PSGP,PSGORD) Q
 I PSGOEAV,+PSJSYSU=3,'$D(PSGOES) D EN^PSGPEN(PSGORD),UNL^PSSLOCK(PSGP,PSGORD) Q:'VFD
 I PSGOEAV,VFD D VFDAV Q  ;Auto-VeRify On
 ;DSS/LM/SMP - END MOD
...
VFD ;DSS/LM - Copy External Rx# and optionally provider comments, if appropriate
 ...
 Q:'$G(PSGORD)!'$G(PSGOORD)  ;Sanity check
 I PSGOORD["P" D VFDP Q
 I PSGOORD["U" D VFDU Q
 Q
VFDAV ;DSS/LM - AUTO-VERIFY: ON
 Q:$$GET^XPAR("SYS","VFD COPY EXTERNAL RX",+$G(DUZ(2)),"I")=0  ;Division check
 Q:'$G(PSGORD)!'$G(PSGOORD)  ;Sanity check
 I PSGOORD["U" D VFDAVU I 1 ;From UD to UD
 E  I PSGOORD["P" D VFDAVP ;From non-verified to UD
 Q
VFDP ;DSS/LM - Continuation of VFD - FROM non-verified TO non-verified (AV=NO)
 N VFDXRX S VFDXRX=$P($G(^PS(53.1,+PSGOORD,21600)),U) ;Rx# from d/c'ed order
 Q:'$L(VFDXRX)  ;order does not have an external Rx#
 S:$E(VFDXRX,$L(VFDXRX))="D" VFDXRX=$E(VFDXRX,1,$L(VFDXRX)-1)
 N VFDCMT
 D:$O(^PS(53.1,+PSGOORD,12,0))
 .N DIR,DIRUT,X,Y
 .S DIR(0)="Y" W !
 .S DIR("A")="External Rx# will be copied. Copy provider comments also"
 .S DIR("B")="Yes"
 .D ^DIR Q:$D(DIRUT)
 .M:+Y VFDCMT=^PS(53.1,+PSGOORD,12)
 .Q
 N VFDFDA S VFDFDA(53.1,+PSGORD_",",21600)=VFDXRX D FILE^DIE(,$NA(VFDFDA))
 ; DSS/LM - Enhancement Request v3 (part 2) - Add suffix to old EXT Rx#
 S $P(^PS(53.1,+PSGOORD,21600),U)=VFDXRX_"D"
 ; DSS/LM - End modification
 Q:'$D(VFDCMT)  M ^PS(53.1,+PSGORD,12)=VFDCMT ;Copy provider comments also
 Q
VFDU ;DSS/LM - Continuation of VFD - FROM unit-dose TO non-verified (AV=NO)
 ;
 I $G(DFN),DFN=+$G(^PS(55,DFN,0)),$G(PSGORD)["P" ;Additional sanity checks
 E  Q
 N VFDXRX S VFDXRX=$$GET1^DIQ(55.06,+PSGOORD_","_+DFN_",",21600) ;Rx# from d/c'ed order
 Q:'$L(VFDXRX)  ;order does not have an external Rx#
 S:$E(VFDXRX,$L(VFDXRX))="D" VFDXRX=$E(VFDXRX,1,$L(VFDXRX)-1)
 ; DSS/LM - Enhancement Request v3 (part 2) - Add suffix to old EXT Rx#
 ;          Variable VFDSRC identifies old UNIT DOSE multiple EXT Rx# IENS
 N VFDCMT,VFDSRC
 S VFDSRC=+PSGOORD_","_+DFN_"," ;Note that this is an IENS, not $NAME
 D:$O(^PS(55,+DFN,5,+PSGOORD,12,0))
 .N DIR,DIRUT,X,Y
 .S DIR(0)="Y" W !
 .S DIR("A")="External Rx# will be copied. Copy provider comments also"
 .S DIR("B")="Yes"
 .D ^DIR Q:$D(DIRUT)
 .M:+Y VFDCMT=^PS(55,+DFN,5,+PSGOORD,12)
 .; 12/2015 - Replace "^"-piece 2 of WP header if not empty
 .I $P($G(VFDCMT(0)),"^",2)]"" S $P(VFDCMT(0),"^",2)=53.1012
 .Q
 N VFDFDA S VFDFDA(53.1,+PSGORD_",",21600)=VFDXRX D FILE^DIE(,$NA(VFDFDA))
 K VFDFDA S VFDFDA(55.06,VFDSRC,21600)=VFDXRX_"D" D FILE^DIE(,$NA(VFDFDA))
 ; DSS/LM - End suffix "D" modification
 Q:'$D(VFDCMT)  M ^PS(53.1,+PSGORD,12)=VFDCMT ;Copy provider comments also
 Q
VFDAVP ;DSS/LM - FROM non-verified TO unit-dose, AUTO-VERIFY: ON
 I $G(DFN),DFN=+$G(^PS(55,DFN,0))
 E  Q  ;DINUM check failed
 N VFDXRX S VFDXRX=$P($G(^PS(53.1,+PSGOORD,21600)),U) ;Rx# from d/c'ed order
 Q:'$L(VFDXRX)  ;order does not have an external Rx#
 S:$E(VFDXRX,$L(VFDXRX))="D" VFDXRX=$E(VFDXRX,1,$L(VFDXRX)-1)
 N VFDCMT
 ...
 .Q
 N VFDFDA S VFDFDA(55.06,+PSGORD_","_+DFN_",",21600)=VFDXRX
 D FILE^DIE(,$NA(VFDFDA)) Q:'$D(VFDCMT)
 ; DSS/LM - Enhancement Request v3 (part 2) - Add suffix to old EXT Rx#
 S $P(^PS(53.1,+PSGOORD,21600),U)=VFDXRX_"D"
 M ^PS(55,+DFN,5,+PSGORD,12)=VFDCMT
 Q
VFDAVU ;DSS/LM - FROM unit-dose TO unit-dose, AUTO-VERIFY: ON
 Q:'(PSGOORD["U"!'(PSGORD["U"))
 N VFDXRX S VFDXRX=$$GET1^DIQ(55.06,+PSGOORD_","_+DFN_",",21600) ;Rx# from d/c'ed order
 Q:'$L(VFDXRX)  ;order does not have an external Rx#
 S:$E(VFDXRX,$L(VFDXRX))="D" VFDXRX=$E(VFDXRX,1,$L(VFDXRX)-1)
 ; DSS/LM - Enhancement Request v3 (part 2) - Add suffix to old EXT Rx#
 ;          Variable VFDSRC identifies old UNIT DOSE multiple EXT Rx# IENS
 N VFDCMT,VFDSRC,VFDTGT
 S VFDSRC=+PSGOORD_","_+DFN_"," ;Note that this is an IENS, not $NAME
 S VFDTGT=+PSGORD_","_+DFN_","
 D:$O(^PS(55,+DFN,5,+PSGOORD,12,0))
 .N DIR,DIRUT,X,Y
 .S DIR(0)="Y" W !
 .S DIR("A")="External Rx# will be copied. Copy provider comments also"
 ...
 .M:+Y VFDCMT=^PS(55,+DFN,5,+PSGOORD,12)
 .Q
 N VFDFDA S VFDFDA(55.06,VFDTGT,21600)=VFDXRX D FILE^DIE(,$NA(VFDFDA))
 ; DSS/LM - Enhancement Request v3 (part 2) - Add suffix to old EXT Rx#
 K VFDFDA S VFDFDA(55.06,VFDSRC,21600)=VFDXRX_"D" D FILE^DIE(,$NA(VFDFDA))
 Q:'$D(VFDCMT)  N DA D DA^DILF(VFDTGT,.DA) ;Recompute subscripts
