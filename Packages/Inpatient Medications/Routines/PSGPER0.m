PSGPER0 ;BIR/CML3-PRINTS PRE-EXCHANGE NEEDS REPORT ;24 JAN 94 / 11:14 AM
 ;;5.0;INPATIENT MEDICATIONS;**58,82,95,115,279**;16 DEC 97;Build 2
 ;
...
DDS ;
 ...
 ;DSS/JCH/SMP - BEGIN MOD - Don't round pre-exchange doses
 I ND#1 D
 .I $G(^%ZOSF("ZVX"))'["VX" S ND=(ND\1)+1 Q
 .Q:$T(FILREQ^PSGPEN)']""
 .N VFD S VFD=$$GET^XPAR("ALL","VFD PSJ FILL ON REQUEST AMOUNT")
 .I '(VFD&($$FILREQ^PSGPEN(+$G(DFN),+$G(ON)))) S ND=(ND\1)+1
 ;DSS/JCH/SMP - END MOD
...
PPI ;
 ...
 ;DSS/AMC/SMP - BEGIN MOD - Print Patient ID as the barcode
 W !!,$S(WD'="zz":WD,1:"NOT FOUND"),?32,RB,!,PN,"  ("_SN_")" ;Q
 I $G(^%ZOSF("ZVX"))["VX",$T(^VFDPSBAR)]"",IOST["P-" D
 .N PID,VA
 .D DEM^VADPT
 .S PID=$S($T(^VFDPSPRT)]"":$$PID^VFDPSPRT(),1:$G(VA("PID")))
 .D BC128^VFDPSBAR(PID,1,50,1800,$Y*50)
