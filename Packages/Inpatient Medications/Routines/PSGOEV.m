PSGOEV ;BIR/CML3-VERIFY (MAKE ACTIVE) ORDERS ; 4/16/10 9:18am
 ;;5.0;INPATIENT MEDICATIONS;**5,7,15,28,33,50,64,58,77,78,80,110,111,133,171,207,241,267,268,260,288,199**;16 DEC 97;Build 1
 ;
...
VFY ; change status, move to 55, and change label record **ENHANCEMENTS MADE IN PSJ*5.0*260 **CCR 6214 **CCR 6244
 ...
 ;DSS/LM/SMP - BEGIN MOD - Copy External Rx# and opt provider comments
 D:$G(^%ZOSF("ZVX"))["VX" VFD
 ;DSS/LM/SMP - END MOD
...
ACTLOG(PSGORDP,DFN,PSGORD)  ;Store 53.1 activity log in local array to be moved to 55
 ...
 ;
 ;DSS/LM - BEGIN MOD
VFD ;DSS/LM - Copy External Rx# and optionally provider comments, if appropriate
 Q:$$GET^XPAR("SYS","VFD COPY EXTERNAL RX",+$G(DUZ(2)),"I")=0  ;Division check
 I $G(DA),$G(DA(1)),$G(PSGORDP),PSGORDP["P",'$D(DA(2)) N VFDIENS S VFDIENS=$$IENS^DILF(.DA)
 E  Q  ;Unanticipated context - Cannot process external Rx#
 N VFDXRX S VFDXRX=$P($G(^PS(53.1,+PSGORDP,21600)),U) ;Rx# from d/c'ed order
 Q:'$L(VFDXRX)  ;order does not have an external Rx#
 ; DSS/LM - Enhancement Request v3 (part 2) - Add suffix to old EXT Rx#
 ;          Variable VFDSRC identifies NON VERIFIED ORDERS file EXT Rx# field location
 N VFDCMT,VFDSRC
 S VFDSRC=$NA(^PS(53.1,+PSGORDP,21600))
 D:$O(^PS(53.1,+PSGORDP,12,0))
 .N DIR,DIRUT,X,Y
 .S DIR(0)="Y" W !
 .S DIR("A")="External Rx# will be copied. Copy provider comments also"
 .S DIR("B")="Yes"
 .D ^DIR Q:$D(DIRUT)
 .M:+Y VFDCMT=^PS(53.1,+PSGORDP,12)
 .; 12/2015 - Replace "^"-piece 2 of WP header if not empty
 .I $P($G(VFDCMT(0)),"^",2)]"" S $P(VFDCMT(0),"^",2)=55.0612
 .Q
 N VFDFDA S:$E(VFDXRX,$L(VFDXRX))="D" VFDXRX=$E(VFDXRX,1,$L(VFDXRX)-1)
 S VFDFDA(55.06,VFDIENS,21600)=VFDXRX D FILE^DIE(,$NA(VFDFDA))
 ; Uncomment the following line to append "D" to the File 53.1 EXT Rx# 
 ; S $P(@VFDSRC,"^")=VFDXRX_"D"
 ; DSS/LM - End suffix "D" modification
 Q:'$D(VFDCMT)  D DA^DILF(VFDIENS,.DA) ;Recompute subscripts
 ; DSS/LM - 12/2015 Correct subscript order in next
 I $G(DA),$G(DA(1)) M ^PS(55,DA(1),5,DA,12)=VFDCMT
 Q
