PSJAC ;BIR/CML3-INPATIENT INFORMATION ;28 Apr 98 / 9:02 AM
 ;;5.0;INPATIENT MEDICATIONS;**8,10,50,127,181,275,279**;16 DEC 97;Build 29
 ;
...
CNV ;
 ...
 ;DSS/BG - BEGIN MOD  - Conditionally substitute MRN for SSN
...
HTWT(DFN) ; Get patient's height and weight from Vitals.
 ...
 . D EN6^GMRVUTL S PSJPHT=$P(X,U,8) D
 . . I VFD D  Q
 . . . S:PSJPHT PSJPHT=$$IN2CM^VFDXLF(PSJPHT,2),PSJPHTD="("_$S($D(PSJY2K):$E($$ENDTC2^PSGMI($P(X,U)),1,10),1:$E($$ENDTC^PSGMI($P(X,U)),1,8))_")"
 . . S:PSJPHT PSJPHT=$J(2.54*PSJPHT,0,2),PSJPHTD="("_$S($D(PSJY2K):$E($$ENDTC2^PSGMI($P(X,U)),1,10),1:$E($$ENDTC^PSGMI($P(X,U)),1,8))_")"
 . S GMRVSTR="WT" D EN6^GMRVUTL S PSJPWT=$P(X,U,8) D
 . . I VFD D  Q
 . . . S:PSJPWT PSJPWT=$$LB2KG^VFDXLF(PSJPWT,2),PSJPWTD="("_$S($D(PSJY2K):$E($$ENDTC2^PSGMI($P(X,U)),1,10),1:$E($$ENDTC^PSGMI($P(X,U)),1,8))_")"
 . . S:PSJPWT PSJPWT=$J(PSJPWT/2.2,0,2),PSJPWTD="("_$S($D(PSJY2K):$E($$ENDTC2^PSGMI($P(X,U)),1,10),1:$E($$ENDTC^PSGMI($P(X,U)),1,8))_")"
 . ;DSS/SMP -END MODS
