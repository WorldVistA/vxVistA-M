PXAIVST ;ISL/JVS,KWP,ESW - GET A VISIT FROM ENCOUNTER NODE ;11/20/02 4:38pm
 ;;1.0;PCE PATIENT CARE ENCOUNTER;**5,9,15,74,111,96,130,124,164,168**;Aug 12, 1996;Build 3
 ;
 ;
 Q
VST ;--CREAT A VISIT
...
VAL ;--VALIDATE ENOUGH DATA
 ...
 ;DSS/RJS - BEGIN MODS  ADDED PROGRAM CODE
 I $G(PXAVISIT) D
 .S (PATIENT,PXAA("PATIENT"))=$P(^AUPNVSIT(PXAVISIT,0),"^",5) S PXAA("ENC D/T")=$P(^AUPNVSIT(PXAVISIT,0),"^",1) S PXAA("HOS LOC")=$P(^AUPNVSIT(PXAVISIT,0),"^",22),PXAA("PROGRAM CODE")=+$G(^AUPNVSIT(PXAVISIT,21600))
 ;DSS/RJS - END MODS
 ;
...
CALL ;--CALL
 ...
 ;DSS/RJS - BEGIN MODS ADDED PROGRAM CODE
 I $G(^%ZOSF("ZVX"))["VX",$G(ORPXAPI("ENCOUNTER",1,"PROGRAM CODE")) D
 .I $G(VISIT),'+$G(^AUPNVSIT(VISIT,21600)) S ^AUPNVSIT(VISIT,21600)=ORPXAPI("ENCOUNTER",1,"PROGRAM CODE")
 .I $G(VISIT),+$G(^AUPNVSIT(VISIT,21600)),+$G(^AUPNVSIT(VISIT,21600))'=ORPXAPI("ENCOUNTER",1,"PROGRAM CODE") S ^AUPNVSIT(VISIT,21600)=ORPXAPI("ENCOUNTER",1,"PROGRAM CODE")
 ;DSS/RJS - END MODS
 ;
  D EN1^PXKMAIN
